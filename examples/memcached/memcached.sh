...
echo "RUNNING: LD_DEBUG=files LD_LIBRARY_PATH=${SYMLIBPATH}:${KALLSYMSPATH} $MEMCACHEDSERVER -u root $@" > /dev/stderr

LD_DEBUG=files LD_LIBRARY_PATH=${SYMLIBPATH}:${KALLSYMSPATH} $MEMCACHEDSERVER -u root $@
